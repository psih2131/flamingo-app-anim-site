<template>
    <main class="main legal-main">
        <div class="container">
            <canvas ref="timeline" width="800" height="100"></canvas>
            <div class="controls">
                <label>Zoom {{ zoomVal }}: <input v-model="zoomVal" type="range" min="1" max="120"></label>
                <label>Scroll {{ scrollVal }}: <input v-model="scrollVal" type="range" min="0" max="691200"></label> <!-- 86400 * 7 -->
            </div>
...
export default {
    ...
    setup() {
        ...
        function generateGeoLog() {
            homeRegion = Math.floor(Math.random() * 3);
            let log = [];
            let awayNights = 0;
                    ...
                    for (let region of tripRegions) {
                        ...
                        while (pointsLeft > 0) {
                            ...
                            for (let i = 0; i < batchSize; i++) {
                                ...
                                currentTime.setSeconds(currentTime.getSeconds() + Math.floor(Math.random() * 10) + 5);
        ...
        function drawTimeline() {
            ...
            let totalWidth = 86400 * 7 * pxPerSecond; // Total width of the timeline in pixels
    ...
    methods: {
        ...
        calcCSV() {
            ...
            for (let i = 0; i < this.log.length; i++) {
                const el = this.log[i];
                if (prev != el.startDate.slice(0,5)) {nextColor += 1;color = this.getColor(nextColor)}
